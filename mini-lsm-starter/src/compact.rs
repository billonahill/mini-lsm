...
use crate::compact::CompactionTask::{ForceFullCompaction, Leveled, Simple, Tiered};
                ...
                );
                {
                    ...
                    for id in l0_sstables.iter() {
                        l0_iters.push(Box::new(SsTableIterator::create_and_seek_to_first(
                            storage_lock.sstables.get(id).unwrap().clone(),
                        )?));
                    }
                    let mut l1_iters = Vec::new();
                    for id in l1_sstables.iter() {
                        l1_iters.push(storage_lock.sstables.get(id).unwrap().clone());
                    }
                    merge_iterator = TwoMergeIterator::create(
                        MergeIterator::create(l0_iters),
                        SstConcatIterator::create_and_seek_to_first(l1_iters)?,
                    )?;
                }

                self.compact_generate_sst_from_iter(merge_iterator, _task.compact_to_bottom_level())
            }
            Simple(_) => {
                unimplemented!()
            }
