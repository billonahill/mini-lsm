...
impl MemTable {
    ...
    pub fn get(&self, _key: &[u8]) -> Option<Bytes> {
        self.map.get(_key).map(|kvp| kvp.value().clone())
    }
    ...
    pub fn reaches_limit_on_write(&self, _key: &[u8], _value: &[u8]) -> bool {
        let max_memtable_size = 100;
        self.approximate_size() + _key.len() + _value.len() > max_memtable_size
    }
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        let entry: (Bytes, Bytes) =
            self.with_iter_mut(|iter| MemTableIterator::entry_to_item(iter.next()));
        self.with_mut(|i| *i.item = entry);
        Ok(())
    }
}
